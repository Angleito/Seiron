/**
 * Comprehensive TypeScript types for Sei Protocol integrations
 * Includes Silo (staking) and Citrex (perpetual trading) protocols
 */
...
export interface SiloStakeParams {
  ...
  token: TokenAddress;
  amount: string;
  stakingPeriod?: number; // in seconds, optional for flexible staking
...
export interface SiloClaimRewardsParams {
  walletAddress: WalletAddress;
  positionId?: string; // if not provided, claims all rewards
  rewardTokens?: TokenAddress[]; // specific tokens to claim
  vestRewards?: boolean;
}
...
export interface SiloStakingPoolInfo {
  ...
  apr: number;
  apy: number;
  lockupPeriods: number[]; // available lockup periods in seconds
  multipliers: Record<string, number>; // lockup period -> multiplier
  penalties: {
    earlyUnstake: number;
    slashing: number;
  };
...
export interface CitrexClosePositionParams {
  walletAddress: WalletAddress;
  positionId: string;
  size?: string; // partial close if specified
...
export interface CitrexLiquidationInfo {
  ...
  liquidationPrice: number;
  marginRatio: number;
  timeToLiquidation: number; // estimated seconds
...
export class SiloProtocolError extends Error {
  constructor(
    message: string,
    public code: string,
    public details?: any
...
export class CitrexProtocolError extends Error {
  constructor(
    message: string,
    public code: string,
    public details?: any
