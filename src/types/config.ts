/**
 * @fileoverview Configuration types for all data collectors and system components
 * Comprehensive configuration schemas for blockchain, exchange, and ML systems
 */
...
export type StorageConfig = BaseConfig & {
  ...
  readonly partitioning: {
    ...
    readonly partition_size: string; // e.g., '1day', '1hour', '1GB'
...
/**
 * Blockchain data collector configurations
 */
...
export type BlockchainNodeConfig = BaseConfig & {
  ...
  readonly caching: {
    ...
    readonly cache_responses: ReadonlyArray<string>; // method names to cache
  };
};
...
export type BlockCollectorConfig = BaseConfig & {
  ...
  readonly processing_pipeline: ReadonlyArray<string>; // pipeline step IDs
};
...
export type TransactionMonitorConfig = BaseConfig & {
  ...
  readonly filters: {
    ...
    readonly value_range?: {
      readonly min: string; // BigInt as string
      readonly max: string; // BigInt as string
    };
...
/**
 * Exchange data collector configurations
 */
...
export type MarketDataCollectorConfig = BaseConfig & {
  ...
  readonly orderbook: {
    readonly depth: number; // number of levels
  ...
  readonly ohlcv: {
    ...
    readonly limit: number; // number of candles per request
  };
  readonly storage: StorageConfig;
};
...
/** Liquidity pool monitor configuration */
...
export type FeatureExtractionConfig = BaseConfig & {
  readonly data_sources: ReadonlyArray<string>; // data source IDs
...
export const createDefaultConfig = (environment: Environment): Partial<SystemConfiguration> => ({
  environment,
...
export const mergeConfigs = <T extends BaseConfig>(
  base: T,
  override: Partial<T>
): T => ({
  ...base,
...
/** Type guard for database config */
export const isDatabaseConfig = (config: unknown): config is DatabaseConfig =>
  typeof config === 'object' &&
  config !== null &&
  'type' in config &&
  'connection' in config;

/** Type guard for exchange config */
export const isExchangeConfig = (config: unknown): config is ExchangeConfig =>
  typeof config === 'object' &&
  config !== null &&
  'exchange_id' in config &&
  'api' in config;

/** Type guard for ML experiment config */
export const isMLExperimentConfig = (config: unknown): config is MLExperimentConfig =>
  typeof config === 'object' &&
  config !== null &&
