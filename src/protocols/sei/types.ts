...
export type SymphonyError = 
  ...
  | { type: 'invalid_token'; token: string; reason: string }
  | { type: 'insufficient_liquidity'; pair: string; requested: string; available: string }
  | { type: 'slippage_exceeded'; expected: string; actual: string; limit: string }
  | { type: 'route_not_found'; tokenIn: string; tokenOut: string; amount: string }
  | { type: 'quote_expired'; quoteId: string; expiredAt: number }
  | { type: 'gas_estimation_failed'; reason: string }
  | { type: 'validation_failed'; errors: string[] }
  | { type: 'execution_failed'; txHash?: string; reason: string }
  | { type: 'timeout'; operation: string; duration: number }
  | { type: 'rate_limit_exceeded'; resetTime: number }
  | { type: 'protocol_unavailable'; protocol: string; reason: string };

export type SymphonyResult<T> = Either<SymphonyError, T>;

// Symphony Protocol Configuration
