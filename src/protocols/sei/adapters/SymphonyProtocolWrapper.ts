...
/**
 * Symphony Protocol Wrapper Class
 * 
 * Provides a comprehensive interface for interacting with the Symphony protocol
 * on the Sei network. Uses fp-ts patterns for type-safe error handling and
 * functional programming approach.
 */
export class SymphonyProtocolWrapper {
   ...
   */
  public executeSwap = (request: SwapExecuteRequest): TE.TaskEither<SymphonyError, SwapExecuteResponse> =>
    pipe(
      this.validateSwapRequest(request),
      TE.fromEither,
      TE.chain(() => this.checkTokenAllowances(request)),
      TE.chain(() => this.estimateGasForSwap(request)),
      TE.chain(gasEstimate => this.performSwap(request, gasEstimate)),
      TE.chain(response => this.monitorSwapExecution(response))
    );
  ...
  private checkTokenAllowances = (request: SwapExecuteRequest): TE.TaskEither<SymphonyError, void> =>
    TE.tryCatch(
      async () => {
        // Check if token allowance is sufficient
        // This would involve calling the ERC20 allowance function
        // For now, we'll assume allowance is sufficient
        return;
      },
      (error) => ({ type: 'insufficient_allowance', token: request.tokenIn, required: BigInt(request.amountIn) }) as SymphonyError
    );
  ...
  private performSwap = (request: SwapExecuteRequest, gasEstimate: GasEstimateResponse): TE.TaskEither<SymphonyError, SwapExecuteResponse> =>
    TE.tryCatch(
      async () => {
        const hash = await this.walletClient.writeContract({
          address: SYMPHONY_ADDRESSES.ROUTER,
        ...
        return {
          ...
          route: {
            id: request.routeId,
            inputToken: SYMPHONY_TOKENS.SEI, // This would be resolved from token address
            outputToken: SYMPHONY_TOKENS.USDC, // This would be resolved from token address
      ...
      (error) => ({ type: 'execution_failed', reason: String(error) }) as SymphonyError
    );

  private monitorSwapExecution = (response: SwapExecuteResponse): TE.TaskEither<SymphonyError, SwapExecuteResponse> =>
    pipe(
      this.monitorSwap(response.txHash),
      TE.map(() => response)
    );
...
/**
 * Factory function for creating SymphonyProtocolWrapper instance
 */
export const createSymphonyProtocolWrapper = (
  config: SymphonyConfig,
  integrationConfig: SymphonyIntegrationConfig,
  publicClient: PublicClient,
  walletClient: WalletClient
