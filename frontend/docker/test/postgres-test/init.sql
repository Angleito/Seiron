...
ALTER SYSTEM SET log_line_prefix = '%t [%p-%l] %u@%d ';
...
BEGIN
    ...
    TRUNCATE TABLE IF EXISTS ai_memory CASCADE;
...
CREATE OR REPLACE FUNCTION test_data.create_test_isolation()
RETURNS void AS $$
BEGIN
    -- Create savepoint for test isolation
    SAVEPOINT test_isolation;
    RAISE NOTICE 'Test isolation savepoint created';
END;
$$ LANGUAGE plpgsql;

-- Function to rollback test isolation
CREATE OR REPLACE FUNCTION test_data.rollback_test_isolation()
RETURNS void AS $$
BEGIN
    -- Rollback to savepoint
    ROLLBACK TO SAVEPOINT test_isolation;
    RAISE NOTICE 'Test isolation rolled back';
END;
$$ LANGUAGE plpgsql;
